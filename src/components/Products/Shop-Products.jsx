...
export default function ShopProducts({
  ...
  useEffect(() => {
    ...
    const filtered = allProducts.filter((product) => {
      ...
      const avgRating = ratings[product.id]?.averageRating || 0;
      if (filters.review && filters.review !== "All") {
        if (filters.review === "5.0 Stars" && avgRating < 5.0) return false;
        if (
          filters.review === "4.0 - 4.9 Stars" &&
          (avgRating < 4.0 || avgRating > 4.9)
        )
          return false;
        if (
          filters.review === "3.0 - 3.9 Stars" &&
          (avgRating < 3.0 || avgRating > 3.9)
        )
          return false;
        if (
          filters.review === "2.0 - 2.9 Stars" &&
          (avgRating < 2.0 || avgRating > 2.9)
        )
          return false;
        if (
          filters.review === "1.0 - 1.9 Stars" &&
          (avgRating < 1.0 || avgRating > 1.9)
        )
          return false;
      }
  ...
  }, [selectedGroup, groupedCollections, allProducts, filters]);
                ...
                <div className="relative">
                  ...
                  <FilterDropdown
                    ...
                    onFiltersApply={(appliedFilters) => {
                      ...
                      setSelectedCollection(appliedFilters.collection);
                    }}
                  />
                </div>
              )}
