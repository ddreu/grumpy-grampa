...
export default function HeroImage() {
  ...
  return (
    <div className="bg-neutral-50 py-12 relative w-full">
      {/* Navigation buttons */}
      <div className="absolute left-26 top-1/2 z-10 -translate-y-1/2">
        <button className="swiper-button-prev bg-black text-white border-2 border-white rounded-full p-3 shadow-lg">
          <ArrowLeft className="w-5 h-5" />
        </button>
      </div>
      <div className="absolute right-26 top-1/2 z-10 -translate-y-1/2">
        <button className="swiper-button-next bg-black text-white border-2 border-white rounded-full p-3 shadow-lg">
      ...
      <Swiper
        ...
        centeredSlides
        loop
        onSlideChange={(swiper) => setActiveIndex(swiper.realIndex)} // 🟡 capture active slide
      ...
      >
        {products.map((product, index) => {
          ...
          const isActive = index === activeIndex; // 🟡 check if it's the center slide

          return (
            <SwiperSlide key={product.id}>
              <div
                className={`relative group overflow-hidden hover:shadow-lg transition-all duration-300 rounded-[5rem] ${
                  isActive
