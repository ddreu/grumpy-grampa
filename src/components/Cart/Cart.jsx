...
export default function CartPage() {
  ...
  // Map Shopify cart lines to your layout format
  const cartItems = (cart?.lines?.edges || []).map(({ node }) => {
    const merchandise = node.merchandise;
    return {
      id: node.id, // CartLine ID
      variantId: merchandise?.id,
      name: merchandise?.title || "Unknown Product",
      price: parseFloat(merchandise?.price?.amount || 0),
      oldPrice: merchandise?.compareAtPrice
        ? parseFloat(merchandise.compareAtPrice.amount)
        : parseFloat(merchandise?.price?.amount || 0),
      variations: (merchandise?.selectedOptions || []).map((opt) => opt.value),
